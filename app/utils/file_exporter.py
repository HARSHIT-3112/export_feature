...
def export_to_docx(content, meta, include_cover_page):
    doc = Document()
    ...
    if include_cover_page:
        ...
        doc.add_paragraph("Generated by LawVriksh Export Service")
        doc.add_page_break()  # new page for content

    # ✅ DOCUMENT BODY
    for el in soup.find_all(["h1", "h2", "h3", "p", "ul", "ol", "li"]):
        if el.name and el.name.startswith("h"):
            level = int(el.name[1]) if el.name[1].isdigit() else 2
            doc.add_heading(el.get_text(strip=True), level=level)
        elif el.name == "p":
            doc.add_paragraph(el.get_text(strip=True))
        elif el.name in ["ul", "ol"]:
            for li in el.find_all("li"):
                doc.add_paragraph("• " + li.get_text(strip=True))
